            ...
            {storyLog.map((entry, idx) => {
                ...
                return (
                    <div key={`${entry.elementId}-${idx}`} className={`flex flex-col border-b border-white/5 last:border-0 ${isCurrent ? '' : 'opacity-75'}`}>
                        {/* Element title */}
                        <div className={`px-4 pt-3 pb-1 text-sm font-semibold ${isCurrent ? 'text-amber-200' : 'text-zinc-400'}`}>
                            <span dangerouslySetInnerHTML={{ __html: element.title }} />
                            {isCurrent && (
                                <span className="ml-2 text-xs text-amber-400/50 font-normal">← most</span>
                            )}
                        </div>

                        {/* Body text */}
                        {bodyText && (
                            <div className={`px-4 pb-2 text-xs leading-relaxed ${isCurrent ? 'text-orange-50/60' : 'text-zinc-600'}`}>
                                {bodyText}
                            </div>
                        )}
                        ...
                        {entry.choiceMade && (
                            <div className="flex items-start gap-1.5 px-4 py-2 text-xs text-zinc-500 bg-amber-900/10">
                                <RiArrowRightSLine size={14} className="mt-0.5 shrink-0 text-orange-600/60" />
