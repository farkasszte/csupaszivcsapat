    ...
    return (
        <div className="flex flex-col gap-6 p-4">
            ...
            <div className="flex flex-col gap-3">
                <div className="flex items-center gap-1.5 text-[10px] text-[#FDF5E6] uppercase tracking-widest px-1">
                    Felfedezett bejegyzések ({components.length})
                </div>

                {components.length === 0 ? (
                    <div className="bg-zinc-800/20 border border-white/5 rounded-xl p-6 text-center">
                        <RiQuestionMark size={24} className="text-[#FDF5E6] mx-auto mb-2" />
                        <div className="text-xs text-[#FDF5E6] italic">Még nincs felfedezett bejegyzés.</div>
                    </div>
                ) : (
                                    ...
                                    <div className="flex gap-3 p-2.5">
                                        <div className="shrink-0 w-14 h-14 rounded-lg overflow-hidden border border-white/10 bg-zinc-950 flex items-center justify-center relative">
                                            {coverUrl ? (
                                                ...
                                                <RiPlayCircleLine size={24} className="text-[#FDF5E6] group-hover:text-[#FDF5E6] transition-colors opacity-80" />
                                            ) : isCharacter ? (
                                                <RiUser3Line size={18} className="text-[#FDF5E6]" />
                                            ) : (
                                                <RiMapPin2Line size={18} className="text-[#FDF5E6]" />
                                            )}
                                        </div>

                                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                                            <h4 className="text-xs font-bold text-[#FDF5E6] group-hover:text-[#FDF5E6] transition-colors truncate">
                                                {comp.name}
                                            </h4>
                                            {descriptionAttr && (
                                                <div
                                                    className="text-[10px] text-[#FDF5E6] mt-0.5 line-clamp-1 leading-relaxed"
            ...
            <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-1.5 text-[10px] text-[#FDF5E6] uppercase tracking-widest">
                        <RiBookLine size={10} />
                ...
                <div className="relative">
                    ...
                    {searchQuery && (
                        <button
                            ...
                            className="absolute inset-y-0 right-0 pr-3 flex items-center text-[#FDF5E6] hover:text-[#FDF5E6] transition-colors"
                        >
                            <RiCloseLine size={16} />
                        </button>
                    )}
                ...
                <div className="grid gap-2">
                    {Object.keys(filteredCategories).length === 0 ? (
                        <div className="text-xs text-[#FDF5E6] italic text-center py-4">Nincs találat a keresésre.</div>
                    ) : (
                        Object.entries(filteredCategories).map(([category, items]) => {
                            ...
                            return (
                                <div key={category} id={`category-${category.replace(/\s+/g, '-')}`} className="flex flex-col gap-1">
                                    ...
                                    >
                                        <span className="text-xs font-semibold text-[#FDF5E6] group-hover:text-[#FDF5E6] transition-colors">
                                            {category} ({items.length})
                                        </span>
                                        <RiArrowDownSLine
                                            size={16}
                                            className={`text-[#FDF5E6] group-hover:text-[#FDF5E6] transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                                        />
                                    </button>
                                    {isExpanded && (
                                            ...
                                            {items.map((link, idx) => (
                                                ...
                                                >
                                                    <div className="flex-1 min-w-0">
                                                        <div className="text-xs font-semibold text-[#FDF5E6] group-hover:text-[#FDF5E6] transition-colors leading-tight flex items-center gap-2">
                                                            {link.nev}
                                                            ...
                                                            {!link.isTelepules && !link.isTerulet && !link.isTanosveny && (link.vedett === 'Védett' || link.vedett === 'Fokozottan védett' || link.vedett === 'Igen') && (
                                                                <span className="px-1.5 py-0.5 rounded-sm bg-emerald-900/40 text-[#FDF5E6] text-[9px] uppercase tracking-wider border border-emerald-800/30">
                                                                    {link.vedett === 'Igen' ? 'Védett' : link.vedett}
                                                                </span>
                                                            )}
                                                        </div>
                                                        {link.latinNev && (
                                                            <div className="text-[10px] text-[#FDF5E6] mt-0.5 italic leading-tight">
                                                                {link.latinNev}
                                                            </div>
                                                        )}
                                                        <div className="text-[10px] text-[#FDF5E6] mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
